...
def is_robomimic_env(env_name):
    """Determine if an env is Robomimic low-dim env."""
...
def _check_dataset_exists(env_name):
    ...
    assert os.path.exists(download_folder), f"Dataset not found at {download_folder}"
...
def get_dataset(env, env_name):
    ...
    for ep in demos:
        ...
        for k in low_dim_keys["low_dim"]:
            obs_list.append(np.array(rm_dataset[f"data/{ep}/obs/{k}"]))
        for k in low_dim_keys["low_dim"]:
            next_obs_list.append(np.array(rm_dataset[f"data/{ep}/next_obs/{k}"]))

        obs = np.concatenate(obs_list, axis=-1)
        next_obs = np.concatenate(next_obs_list, axis=-1)
...
class RobomimicLowdimWrapper(gym.Env):
    ...
    def __init__(
        ...
        low = np.full_like(obs_example, fill_value=-1.0, dtype=np.float32)
        high = np.full_like(obs_example, fill_value=1.0, dtype=np.float32)
