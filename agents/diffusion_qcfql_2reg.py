...
class ACFQLAgent:
    ...
    def __init__(
        ...
        # set teacher model (diffusion policy)
    ...
    def actor_loss(self, batch: Dict[str, Tensor]) -> Tuple[Tensor, Dict[str, float]]:
        ...
        if self.config['actor_type'] == 'distill-ddpg':
            ...
            # 3) diffusion teacher anchor (있을 때만)
            if (self.teacher_model is not None and 
                self.config.get('lambda_teacher', 0.0) > 0.0):
                with torch.no_grad():
                    teacher_actions = self.sample_teacher_actions(
                        batch['observations']
                    )  # (B, full_action_dim)

                # actor_actions와 차원 맞추기 (chunking 여부에 따라)
                if self.config['action_chunking']:
                    # actor_actions: (B, full_action_dim) 라고 가정
                    # teacher_actions: (B, full_action_dim)
                    assert actor_actions.shape == teacher_actions.shape, \
                        f"actor {actor_actions.shape}, teacher {teacher_actions.shape}"
                teacher_anchor_loss = F.mse_loss(actor_actions, teacher_actions)
        ...
        lambda_teacher = self.config.get('lambda_teacher', 0.0)

        actor_loss = (
            bc_flow_loss +
            self.config['alpha'] * distill_loss +
            q_loss +
            lambda_teacher * teacher_anchor_loss
        )
        ...
        return actor_loss, {
            ...
            'teacher_anchor_loss': teacher_anchor_loss.item(),
        }
    ...
    @torch.no_grad()
    def sample_teacher_actions(self, observations: Tensor) -> Tensor:
        """
        Pretrained diffusion teacher에서 action chunk를 샘플링해서
        ACFQL의 full_action_dim (H * action_dim) 형태의 텐서로 리턴.
        """
        assert self.teacher_model is not None, "teacher_model is None"
        
        # obs shape 정리 (여기서는 마지막 step만 cond로 쓰는 예시)
        # batch['observations']가 (B, T, obs_dim)이면 마지막 step 사용:
        if observations.dim() == 3:
            obs_cond = observations[:, -1, :]    # (B, obs_dim)
        else:
            obs_cond = observations              # (B, obs_dim)

        # (TODO) 실제 dppo DiffusionModel의 sample / sample_actions API에 맞춰 수정
        # 예: (B, horizon_steps, action_dim)
        teacher_actions_seq = self.teacher_model.sample_actions(
            obs_cond,
            horizon_steps=self.config['horizon_length'],
        )

        # (B, H, A) → (B, H*A) 로 flatten
        if teacher_actions_seq.dim() == 3:
            B, H, A = teacher_actions_seq.shape
            teacher_actions = teacher_actions_seq.reshape(B, H * A)
        else:
            teacher_actions = teacher_actions_seq  # 이미 flatten 되었다고 가정

        return torch.clamp(teacher_actions, -1.0, 1.0)
